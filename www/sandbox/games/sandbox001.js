...
import { Graine, Graine1, Graine2, Graine3 } from '../food/graine.js'
...
export class SandBox001 extends Game{
    speed=1
    scene
    textureLoader
    ground
    ...
    previousTime = 0;
    // admin=new Admin();
    constructor(name="SandBox 001",description="Fourrager : récolter des graines"){
        super(name,description,Ant_Messor.description)
    ...
    start(){
        ...
        let colony=new Colony(this.player,this,"Colonie 1")
        this.foods=new Foods(this.ground.obj,"Végétaux")
        this.foods.setGame(this)
        ...
        colony.loadGlbAndTextures([Ant_Messor]).then(e=>{
            ...
            this.foods.setTextureLoader(this.textureLoader)
            const radiusfood=4
            const gameFoodTypes=[Graine1,Graine2,Graine3]
            this.foods.loadGlbAndTextures(gameFoodTypes).then(e=>{
                for(let i=0;i<100;i++){
                    let rand=Math.random()
                    let typ=rand<.4?Graine1:rand<.75?Graine2:Graine3
                    // let typ=i===0?Graine2:Graine1
                    let graine=this.foods.add(typ)
                    let posX=(Math.random()-.5)*radiusfood*2
                    let posZ=(Math.random()-.5)*radiusfood*2
                    graine.setPosition(posX,this.ground.getYFromXZ(posX,posZ),posZ)
                    // graine.followSlope()
                }
                // colony.setTarget(graine)
                this.isReady(()=>{
                ...
                this.panel=PanelsController.createPanel(this.regisseur,this.cameras,colony,this.foods)
